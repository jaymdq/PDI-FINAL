...
public class Imagen {
	...
	public static int getPixel (ImagePlus im, int x, int y){
		...
		Color c = new Color(im.getProcessor().getPixel(y, x));
		return c.getBlue();
		//return c.getGreen();
		//return c.getRed();
	}
	...
	public static Vector<Elemento> segmentar(ImagePlus im){
		Vector<Elemento> segmentos = new Vector<Elemento>();

		for (int y = 0 ; y < im.getHeight() ; y++){
			for (int x = 0 ; x < im.getWidth() ; x++){
				if (im.getProcessor().getPixel(x, y) != 0 ){
					//Estoy en el objeto.
					int xini = x;
					x++;
					while ( im.getProcessor().getPixel(x, y) != 0 && x < im.getWidth()){
						x++;
					}
					int xfin = x;
					Elemento aIngresar = new Elemento(y,xini,xfin);
					segmentos.add(aIngresar);
					//System.out.println("Salida: y: "+y+" xi: "+xini+" yf: "+xfin);
				}		
			}
		}
		return segmentos;	
